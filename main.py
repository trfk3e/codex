...
def build_slug_consistency_prompt(
    ...
    h1 = (h1 or "").strip()
    body = [
        "Ты профессионал в своем деле, будь очень внимателен. Проверь, отражает ли /slug язык секции?\n",
        "SLUG пишут без диакритики — это нормально, если он всё равно про ту же тему. Если кириллица - /slug должен быть строго на английском, если это правило НЕ соблюдено  — ответь «Нет». Иначе ответь «Да».\n.\n",
        "Если SLUG заметно на другом языке — ответь «Нет». Иначе ответь «Да».\n",
        "Если в /slug написана билиберда или бред, или он отличается языком — ответь «Нет». Иначе ответь «Да».\n"
        "В /slug - все адреса начинаются с '/'. Если это правило НЕ соблюдено  — ответь «Нет». Иначе ответь «Да».\n"  
        "В /slug - после '/' идут слова через дефис '-', без пробелов. Если это правило НЕ соблюдено  — ответь «Нет». Иначе ответь «Да».\n"  
        "В /slug - если язык латиница — переводить на этот язык. Если это правило НЕ соблюдено  — ответь «Нет». Иначе ответь «Да».\n"  
        "В /slug - если язык нелатиница (греческий, кириллица, иероглифы, арабский и др.) — использовать английский оригинал СТРОГО ВСЕГДА. Если это правило НЕ соблюдено  — ответь «Нет». Иначе ответь «Да».\n"  
        "Прочитай внимательно что написано в правилах выше. Отвечай строго одним словом «Да» или «Нет». Никаких пояснений.",
        "",
        "<SLUG>",
